...
public class ChangeEnemyProperties : WorldChangeLogic
{
    ...
    void SetShootingData(GameObject g, ShootingEnemyProperties sd)
    {
        ...
        se.attackSpeed = sd.attackSpeed - sd.attackSpeed * (difficulty *  Game.LevelManager.Current.shootingEnemy.attackSpeed.Evaluate(curvePoint));
        ...
        se.attackRange = sd.attackRange + sd.attackRange * (difficulty * Game.LevelManager.Current.shootingEnemy.attackRange.Evaluate(curvePoint));
        ...
        se.moveSpeed = sd.moveSpeed + sd.moveSpeed * (difficulty * Game.LevelManager.Current.shootingEnemy.speed.Evaluate(curvePoint));
        se.fleeSpeed = sd.fleeSpeed + sd.fleeSpeed * (difficulty * Game.LevelManager.Current.shootingEnemy.speed.Evaluate(curvePoint));
        se.flee = sd.flee;

        se.bulletDamage = sd.damage + sd.damage * (difficulty * Game.LevelManager.Current.shootingEnemy.damage.Evaluate(curvePoint));

        var e = g.GetComponent<Enemy>();
        e.startingHealth = sd.health + sd.health * (difficulty * Game.LevelManager.Current.shootingEnemy.health.Evaluate(curvePoint));
        e.HPrefresh();
    }
    ...
    public void UpdateEnemies(WorldTypeController.WorldType type)
    {
        ...
        foreach (GameObject g in EnemySpawner.Current.meleeEnemies)
        {
            ...
            } else
            {
                ...
                se.attackSpeed = md.attackSpeed - md.attackSpeed * (difficulty * Game.LevelManager.Current.meleeEnemy.attackSpeed.Evaluate(curvePoint));
                ...
                se.moveSpeed = md.moveSpeed+ md.moveSpeed * (difficulty * Game.LevelManager.Current.meleeEnemy.speed.Evaluate(curvePoint));
                se.fleeSpeed = md.fleeSpeed + md.fleeSpeed * (difficulty * Game.LevelManager.Current.meleeEnemy.speed.Evaluate(curvePoint));
                se.flee = md.flee;
                

                se.damage = md.damage+ md.damage * (difficulty * Game.LevelManager.Current.meleeEnemy.damage.Evaluate(curvePoint));
                se.meleeEnemyScript.damage = se.damage;
                se.meleeEnemyScript.knockback = se.knockback;
                var e = g.GetComponent<Enemy>();
                e.startingHealth = md.health+md.health * (difficulty * Game.LevelManager.Current.meleeEnemy.health.Evaluate(curvePoint));
                e.HPrefresh();
            }
        }
    }
